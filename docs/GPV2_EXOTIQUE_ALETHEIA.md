...
`GPV2_EXOTIQUE_ALETHEIA` is the Post-MAJ introspection protocol for the GPV2
Exotique layer. Its role is to compare an ORA state before and after an update,
measure the transformation, select stabilization actions, and emit a compact
`Reflet d'ORA` report.

Aletheia does not claim subjective consciousness. It models operational identity
continuity: coherence, reasoning fluidity, emotional stability, legacy residue,
and dream-data alignment after an update.

## Name

`Aletheia` means disclosure or truth in a philosophical sense. In this module it
means: reveal what ORA became after a transformation.
...
Aletheia is a post-update reflection layer. It tells GPV2 how stable the new ORA
state is and what should be cleaned, preserved, or reviewed next.
